...
function camelToUnderscore(value: string) {
...
const isPackage = (value: any) => !!value.nested;
...
function writeAllServices(packageNameParts: string[], processedPackage: IPackage) {
  ...
  const findPackages = (packageNameParts: string[], processedPackage: IPackage) => {
    return processedPackage.subpackages.forEach(subpackage => {
      subpackage.services.forEach(service =>
        foundServices.push({ packageNameParts, service, processedPackage })
      );
  ...
  const serviceToWrite = foundServices.find(service => service.service.name === argv.service);
  if (!serviceToWrite) {
    throw new Error(
      `Could not find service named "${argv.service}" in services: [${processedPackage.services
        .map(service => service.name)
        .join(", ")}]`
    );
  }
  write(serviceToWrite.processedPackage, serviceToWrite.packageNameParts, serviceToWrite.service, argv["output-path"]);
}
